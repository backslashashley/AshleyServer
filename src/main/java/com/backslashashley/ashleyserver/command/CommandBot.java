...
import com.backslashashley.ashleyserver.bots.BotServerPlayerEntity;
...
public class CommandBot extends CommandBase {
	public String getName() {
		return "bot";
	}

	public String getUsage(CommandSource source) {
		return "bot <player_name> <spawn|kill> ";
	}

	public void run(MinecraftServer server, CommandSource source, String[] args) throws CommandException {
		if (args.length < 2) {
			throw new IncorrectUsageException("bot <x> <kill|spawn>");
		}
		...
		if (source instanceof PlayerEntity) {
			...
			if (!(server.getPlayerManager().isOp(sendingPlayer.getGameProfile()))) {
				if (!(sendingPlayer == player || player == null || player instanceof BotServerPlayerEntity)) {
		...
		if ("spawn".equalsIgnoreCase(action)) {
			...
			if (playerName.length() < 3 || playerName.length() > 16) {
				throw new IncorrectUsageException("bot names can only be 3 to 16 chars long");
			}
			...
			BotServerPlayerEntity.createBot(playerName, server, d0, d1, d2, yaw, pitch, dimension, 3);

			return;
		}
		if ("kill".equalsIgnoreCase(action)) {
			if (!(player instanceof BotServerPlayerEntity)) {
